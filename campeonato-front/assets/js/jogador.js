        ...
        ({ id_jogador, nome, numero, posicao, data_nascimento, sexo }, i) => {
            ...
            <tr data-key="${id_jogador}">
                ...
                <td class='d-flex gap-2'>
                    ...
                    <button class="btn btn-primary btn-sm" onclick="handleBuildModalForm(${Number(id_jogador)})">Editar</button>
...
async function handleDetails(id) {
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <p>Número: <span class="badge bg-light text-dark">${item.numero}</span></p>
            <p>Data de Nascimento:  <span class="badge bg-light text-dark">${formatarDataEmNumero(item.data_nascimento)}</span></p>
            <p>Sexo: <span class="badge bg-light text-dark">${item.sexo}</span></p>
            Equipa:
            <div class="d-flex gap-1 ">
            ${item?.equipa ? `<span class="badge bg-secondary">${equipa.nome}</span>` : ''}
            </div>
        </div>
    </div>`
    openModal();
...
async function handleBuildModalForm(id) {
    ...
    <form  id="registrationForm" class="row g-2 needs-validation" novalidate onsubmit="handleValidation(event)">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" id="posicao" name="posicao" value="${item?.posicao ?? ''}" required>
            <span class="invalid-feedback">Por favor, insira a posicão.</span>
        </div>

        <div class="col-md-6">
            <label for="numero" class="form-label">Número</label>
            <input type="number" class="form-control" id="numero" name="numero" value="${item?.numero ?? ''}" required>
        ...
        <div class="col-md-6">
            ...
            <input type="date" class="form-control" id="data_nascimento" name="data_nascimento" value="${item?.data_nascimento ?? ''}" required>
