...
async function initialCampeonatoView() {
    ...
    campeonatos.map(
        ({ id_campeonato, nome, data_inicio, data_fim, descricao, valor_pagar, id_foto }, i) => {
            ...
            <tr data-key="${id_campeonato}">
                ...
                <td class='d-flex gap-2'>
                    ...
                    <button class="btn btn-primary btn-sm" onclick="handleBuildModalForm(${Number(id_campeonato)})">Editar</button>
...
async function handleDetails(id) {
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <p>Valor a pagar: <span class="badge bg-light text-dark">${item.valor_pagar} KZ</span></p>
            <p>De  <span class="badge bg-light text-dark">${formatarDataEmNumero(item.data_inicio)}</span> até <span class="badge bg-light text-dark">${formatarDataEmNumero(item.data_fim)} </span></p>
            Equipas:
            <div class="d-flex gap-1 ">
            ${item.equipas.map((equipa) => (
...
async function handleBuildModalForm(id) {
    ...
    const formTamplete = `
    <h4 class="fw-bold text-center mb-3">${id ? 'Editar' : 'Novo'} Campeonato</h4>
    ...
    <form  id="registrationForm" class="row g-2 needs-validation" novalidate onsubmit="handleValidation(event)">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" id="nome" name="nome" value="${item?.nome_campeonato ?? ''}" required>
            <span class="invalid-feedback">Por favor, insira o nome.</span>
        </div>

        <div class="col-md-6">
            <label for="valor_pagar" class="form-label">Valor a Pagar</label>
            <input type="number" class="form-control" id="valor_pagar" name="valor_pagar" value="${item?.valor_pagar ?? ''}" required step="0.01">
            <span class="invalid-feedback">Por favor, insira o valor a pagar.</span>
         </div>

        <div class="col-md-6">
            <label for="data_inicio" class="form-label">Data de Início</label>
            <input type="date" class="form-control" id="data_inicio" name="data_inicio" value="${item?.data_inicio ?? ''}" required>
            <span class="invalid-feedback">Por favor, insira a data de início.</span>
        </div>

        <div class="col-md-6">
            <label for="data_fim" class="form-label">Data de Fim</label>
            <input type="date" class="form-control" id="data_fim" name="data_fim" value="${item?.data_fim ?? ''}" required>
            <span class="invalid-feedback">Por favor, insira a data de fim.</span>
        </div>

        <div class="col-md-6">
            <label for="descricao" class="form-label">Descrição</label>
            <textarea class="form-control" id="descricao" name="descricao" rows="3" required>${item?.descricao_campeonato ?? ''}</textarea>
...
async function handleValidation(event) {
    ...
    if ((dataFim <= dataInicio)) {
        ...
        document.getElementById('data_fim').nextElementSibling.innerHTML = 'A data de fim deve ser posterior à data de início.';
