...
async function initialEstadioView() {
    ...
    estadios.map(
        ({ id_estadio, nome, capacidade, localizacao }, i) => {
            ...
            <tr data-key="${id_estadio}">
                ...
                <td class='d-flex gap-2'>
                    ...
                    <button class="btn btn-primary btn-sm" onclick="handleBuildModalForm(${Number(id_estadio)})">Editar</button>
...
async function handleDetails(id) {
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <p>Capacidade: <span class="badge bg-light text-dark">${item.capacidade ?? 0} pessoas</span></p>
            Equipa:
...
async function handleBuildModalForm(id) {
    ...
    <form  id="registrationForm" class="row g-2 needs-validation" novalidate onsubmit="handleValidation(event)">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" id="localizacao" name="localizacao" value="${item?.localizacao ?? ''}" required>
            <span class="invalid-feedback">Por favor, insira o localizacão.</span>
        </div>

        <div class="col-md-6">
            <label for="capacidade" class="form-label">Capacidade</label>
            <input type="number" class="form-control" id="capacidade" name="capacidade" value="${item?.capacidade ?? ''}" required>
